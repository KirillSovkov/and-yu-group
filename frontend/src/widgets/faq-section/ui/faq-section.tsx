...
const faqList: FaqItemI[] = [
  {
    ...
    content:
      "С дизайн проектом комплектация бесплатная, без проекта — 250 руб/м²",
  },
  {
    key: 2,
  ...
  },
  {
    ...
    content:
      "У нас нет ограничений, мы работаем с любым бюджетом которым располагает наш клиент, от эконом до премиум сегмента",
  },
  {
    key: 4,
    title: "Если материал испортили при доставке? Например разбили ванну?",
    content:
      "Мы несем полную ответственность до момента доставки материалов на объект",
  },
  {
    key: 5,
    title: "Я дизайнер. Как вы сотрудничаете?",
    content:
      "Дизайнеры — это наши партнеры. Для дизайнеров мы предоставляем особые условия индивидуально. Если вы дизайнер, свяжитесь с нами",
  },
  {
    key: 6,
    title: "А если мне не понравятся ваши цены?",
    content:
      "Мы гарантируем в спецификации честные рекомендованные розничные цены. Никаких наценок. При большом объеме предоставим скидку",
  },
  {
    key: 7,
    title: "Если у меня нет дизайн проекта, могу ли я с вами работать?",
    content:
      "Если вы делаете ремонт без дизайн проекта, то нам от вас понадобятся только размеры помещений и ваши пожелания. Наши специалисты займутся подбором материалов для вашего интерьера",
  },
  {
    key: 8,
    title: "Сколько вариантов будет в спецификации?",
    content:
      "Мы стараемся предлагать от 2-х вариантов в разном ценовом сегменте. Чтобы была возможность выбора",
  },
];
  ...
  return (
    <section>
      <div className="flex flex-col items-center justify-center py-10 px-11 gap-9 bg-foreground-100">
        ...
        <Button children={"Обсудить проект"} onPressHandle={openModal} />
      </div>
    </section>
  );
}
